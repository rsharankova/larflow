...
def make_training_plot( logfile, outputpath ):
    ...
    for l in loglines:
        ...
        if "train aveloss" in l:
            pt = ( int(filter(str.isdigit,data[1])), float(re.findall("\d+\.\d+",data[3])[0]), float(re.findall("\d+\.\d+",data[5])[0]) )
            current_epoch = pt[0]
            train_pts.append(pt)
        if "Test:Result*" in l:
            pt = ( current_epoch, float(data[6]), float(data[4]) )
            test_pts.append(pt)
        if "lr=" in l:
